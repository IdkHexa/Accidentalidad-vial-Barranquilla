...
4. **Persiste los datos**: Los registros limpios se guardan en una base de datos SQLite usando el patrón Repositorio (DAO) para que estén listos para ser consultados por la interfaz web.
...
- `config.py` — Configuración global (URL de la API y llave de Google Maps).
...
La descripción técnica detallada de cada fase del proyecto está en la carpeta `docs/`:
- `docs/Entregable1.md` — Módulos 1 y 2: Ingesta, limpieza y geocodificación.
- `docs/Entregable2.md` — Módulos 3 y 4: Persistencia y repositorio DAO.
- `docs/ENTREGABLES.md` — Plan general de entregables y delimitación técnica.
- `docs/METODOLOGIA.md` — Roles del equipo y acuerdos de colaboración.
...
- **SQLAlchemy + SQLite** gestionan la persistencia con mapeo objeto-relacional (ORM).
- **Patrón Repositorio (DAO)** aísla la lógica de base de datos para que el resto del código no dependa de SQLite directamente.
